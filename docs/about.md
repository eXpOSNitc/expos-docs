...
1.  Keep the upward and downward interfaces of the OS kernel idealistically simple – This means:
    ...
    2.  The application programming interface ([API](os-spec/index.md)) (and the application binary interface [­(ABI)](abi.md) ) provided by the OS are kept simple and minimal.
2.  The [OS specification](os-spec/index.md) has been kept as simple as possible from the viewpoint of the OS programmer. The OS features are minimal, some of them being:
...
The eXpOS package simplifies this by providing a hypothetical simple-to-program hardware abstraction – the [XSM Machine](arch-spec/index.md). What makes XSM simple is that its memory words can store an arbitrary string or a number. This means that data formatting – one of the major technicalities while working with machines - is simplified. Further simplifications include the assumption that each disk block fits into exactly one memory page etc. The OS code can be written by programming the machine using a simple to learn [SPL language](support-tools/spl.md) (which is just an enriched XSM assembler).

The second interface of the OS is its upward interface – the interface to application programs. This interface, called the application binary interface ( [­ABI](abi.md) ), defines the low level system call interface between the application and the OS, the virtual address space model for an application, the format of executable files etc. The ABI too is kept minimal. It is easy to load executables into the memory. Only small programs are permitted. Parameter passing between applications and the system calls is also extremely simple.

The eXpOS [­ABI](abi.md) is too primitive to support application programs in the C programming language. Hence we provide a separate language (called [ExpL](support-tools/expl.md)) using which applications can be created and compiled. The ExpL compiler given to the student supports the eXpOS high level application program interface ([API](os-spec/index.md)). We note here that, implementing an ExpL compiler for the eXpOS ABI on the XSM machine would be an instructive exercise for an undergraduate compiler design laboratory. (See [ExpL compiler project](https://silcnitc.github.io).)
