...
5.  Initialize the [disk map table](../os-design/process-table.md#disk_map_table) entry of the shell process (PID = 2) as follows - Store the block numbers of the two code pages in the disk map table entry of the shell. Invalidate all other entries of the disk map table entry by storing -1.
