...
<!--
When the data finally arrives, <b>the console interrupt handler must transfer the data (in port P0) into the input buffer of the process</b> in the process table. Then the handler wakes up process waiting for the terminal.
